...
Public Class IUGenerarContratos
    ...
    Public Sub Buscar_Contrato(ByVal Busqueda As String)
        ...
        dr = IUPrincipal.Conexion.Seleccionar("SELECT * FROM `CONTRATOS` WHERE `Txt_Fk_Cedula` = '" + Busqueda + "' ; ")
        If (dr.Read()) Then
    ...
    Private Sub TlsbtnImprimir_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TlsbtnImprimir.Click
        ...
        Else
            ...
            FechaNacimiento = IUPrincipal.Personas.Item(CInt(LblPosicion.Text)).ProFechaNacimiento
            ...
            ParametroDireccion.Value = IUPrincipal.Personas.Item(CInt(LblPosicion.Text)).ProDireccion
            ...
            Else
                ...
                If CmbContrato.SelectedIndex = 1 Then
    ...
    Private Sub Guardar()
        ...
        If EstadoFormulario = "Agregando" Then
            ...
            If bConexionExitosa Then
                IUPrincipal.Conexion.Insertar("INSERT INTO `CONTRATOS` set `Txt_Descripcion` = '" & IUPrincipal.Contrato.ProDescripcion & "',`Int_Fk_Codigo_Departamento` = '" & CmbDepartamento.SelectedIndex + 1 & "',`Int_Fk_Codgio_Cargo` = '" & IndiceCargos(CmbCargo.SelectedIndex) & "',`Dbl_JornadaDiaria` = '" & IUPrincipal.Contrato.ProJornadaDiaria & "',`Dbl_Sueldo_Mensual` = '" & IUPrincipal.Contrato.ProSueldo & "',`Dte_Fecha_Inicial` = '" + IUPrincipal.Contrato.ProFechaInicial.ToString("yyyy-MM-dd") + "',`Dte_Fecha_Final` = '" & IUPrincipal.Contrato.ProFechaFinal.ToString("yyyy-MM-dd") & "',`Int_Duracion` = '" & IUPrincipal.Contrato.ProDuracion & "',`Txt_Fk_Cedula` = '" + IUPrincipal.Contrato.ProFk_Cedula + "',`Txt_Fk_Cedula_Cubrir` = '" + IUPrincipal.Contrato.ProFk_Cedula_Cubrir + "';")
                IUPrincipal.Conexion.Insertar("UPDATE `PERSONAS` set `Int_Estatus` = '4' where `Txt_Cedula`= '" + IUPrincipal.Contrato.ProFk_Cedula + "';")
            End If
    ...
    Private Sub BotonSiguiente_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BotonSiguiente.Click
        If PosicionActual = IUPrincipal.Contratos.Count Then
            MostrarContrato(IUPrincipal.Contratos.Item(PosicionActual).ProFk_Cedula)
        Else
            MostrarContrato(IUPrincipal.Contratos.Item(PosicionActual + 1).ProFk_Cedula)
        End If
    End Sub
    Private Sub MostrarContrato(ByVal Posicion As String)
        ...
        If bConexionExitosa Then
            ...
            LblDatoCedula.Text = IUPrincipal.Contratos.Item(pos).ProFk_Cedula
            ...
            LblTipoEmp.Text = IUPrincipal.Contratos.Item(pos).ProDescripcion
            ...
            dr = Conexion.Seleccionar("SELECT * FROM `departamentos` WHERE `Num_Codigo` = (SELECT `Num_Codigo_Departamento` from `cargos` WHERE `Num_Codigo` = '" & IUPrincipal.Contratos.Item(pos).ProCargo & "') ; ")
            dr.Read()
            indicedpto = dr(0)
            dr.Dispose()
            dr.Close()
            ...
            LblCargoEmp.Text = Trim(IUPrincipal.Cargos(IUPrincipal.Contratos.Item(pos).ProCargo).Prodescripcion)
            CmbContrato.SelectedItem = IUPrincipal.Contratos.Item(pos).ProDescripcion
            ...
            If IUPrincipal.Contratos.Item(pos).ProTipoHorario = "2" Then
                LblHorarioEmp.Text = "Rotativo"
            ElseIf IUPrincipal.Contratos.Item(pos).ProTipoHorario = "1" Then
                LblHorarioEmp.Text = "Fijo"
            End If

            TxtJornada.Text = IUPrincipal.Contratos.Item(pos).ProJornadaDiaria
            TxtSueldoMensual.Text = IUPrincipal.Contratos.Item(pos).ProSueldo
            TxtDuracion.Text = IUPrincipal.Contratos.Item(pos).ProDuracion
            LblCedulaCubrir.Text = IUPrincipal.Contratos.Item(pos).ProFk_Cedula_Cubrir

            If LblCedulaCubrir.Text <> "" Then
                dr = Conexion.Seleccionar("SELECT * FROM `personas_temp` WHERE `Txt_Cedula` = '" + LblCedulaCubrir.Text + "' ; ")
                dr.Read()
                NombreCubrir = dr(3) + " " + dr(4)
                dr.Dispose()
                dr.Close()
            ...
            DteFechaInicio.Value = IUPrincipal.Contratos.Item(pos).ProFechaInicial
            DteFechaFinal.Value = IUPrincipal.Contratos.Item(pos).ProFechaFinal
        End If
        CmbContrato.Visible = False
        CmbDepartamento.Visible = False
        CmbCargo.Visible = False
